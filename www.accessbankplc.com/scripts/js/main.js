  ...
  return $(window).on("mousemove touchmove", function (e) {
    ...
    if (is_dragging) {
      ...
      $(".payday-calc-float-amount").html(
        "N" + (angle * 1000 * 7.5).toLocaleString("en")
      );
      // set monthly salary
      return $(".loan-amount").html("N" + (angle * 10000).toLocaleString("en"));
    }
  });
});
...
$(function () {
    ...
    //Amex Card Landing Page

    var premiumCardSwiper = new Swiper(".service-container", {
        slidesPerView: 1,
        spaceBetween: 18,
        autoplay: {
            delay: 5000,
    ...
    $("#explore-premium-tab").on("shown.bs.tab", function (e) {
        var paneTarget = $(e.target).attr("href");
        var $thePane = $(".tab-pane" + paneTarget);
        var paneIndex = $thePane.index();
        if (
            $thePane.find(".service-container").length > 0 &&
            0 === $thePane.find(".swiper-slide-active").length
        ) {
            premiumCardSwiper[paneIndex].update();
        }
    });
    $("#explore-gold-tab").on("shown.bs.tab", function (e) {
        var paneTarget = $(e.target).attr("href");
        var $thePane = $(".tab-pane" + paneTarget);
        var paneIndex = $thePane.index();
        if (
