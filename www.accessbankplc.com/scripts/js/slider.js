    ...
    */
    ...
    $('[data-js="city-slider"]').each(function () {
        ...
        images.on('loaded', function () {
            var image = $(this);
            var slide = image.closest('.access-slide');
...
function slideshowSwitch(slideshow, index, auto) {
    ...
    var slides = slideshow.find('.access-slide'),
    ...
    if (transition === 'fade') {
        ...
        newSlide.css({ display: 'block', width: 0, right: newSlideRight, x: newSlideLeft, zIndex: 2 });
        newSlideImage.css({ width: slideshow.width(), right: newSlideImageRight, x: newSlideImageLeft });
        // newSlideContent.css({width: slideshow.width(), x: newSlideContentLeft, right: newSlideContentRight});
        activeSlideImage.css({ x: 0 });

        TweenMax.set(newSlideElements, { y: 20, force3D: true });
        TweenMax.to(activeSlideImage, 1, { x: activeSlideImageLeft, ease: Expo.easeInOut });
        TweenMax.to(newSlide, 1, { width: slideshow.width(), ease: Expo.easeInOut });
        TweenMax.to(newSlideImage, 1, {
            right: newSlideImageToRight,
            x: newSlideImageToLeft,
            ease: Expo.easeInOut
        });
        ...
        TweenMax.staggerFromTo(newSlideElements, 0.8, { alpha: 0, y: 60 }, {
            ...
            newSlide.addClass('is-active').removeClass('is-new');
            activeSlide.removeClass('is-active');
            newSlide.css({ display: '', width: '', x: '', zIndex: '' });
            newSlideImage.css({ width: '', right: '', x: '' });
            newSlideContent.css({ width: '', x: '' });
            newSlideElements.css({ opacity: '', transform: '' });
            activeSlideImage.css({ x: '' });
            slideshow.find('.pages').trigger('check');
            slideshow.data('wait', false);
            if (auto) {
                timeout = setTimeout(function () {
...
function slideshowNext(slideshow, previous, auto) {
    var slides = slideshow.find('.access-slide'),
        ...
        newSlide = null;
    if (previous) {
        newSlide = activeSlide.prev('.access-slide');
        if (newSlide.length === 0) {
            newSlide = slides.last();
        }
    } else {
        newSlide = activeSlide.next('.access-slide');
        if (newSlide.length === 0) {
            newSlide = slides.filter('.access-slide').first();
        }
    }
